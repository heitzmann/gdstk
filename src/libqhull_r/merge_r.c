/*<html><pre>  -<a                             href="qh-merge_r.htm#TOC"
  >-------------------------------</a><a name="TOP">-</a>
   ...
   To remove deleted facets and vertices (qhull() in libqhull_r.c):
     ...
     qh_resetlists(qh, False, qh_RESETvisible);       // qh.visible_list newvertex_list
  newfacet_list
...
const char *mergetypes[] = {
    "none",    "coplanar",  "anglecoplanar", "concave",         "concavecoplanar",
    "twisted", "flip",      "dupridge",      "subridge",        "vertices",
    "degen",   "redundant", "mirror",        "coplanarhorizon",
};
...
*/
void qh_premerge(qhT *qh, int apexpointid, realT maxcentrum,
                 realT maxangle /* qh.newfacet_list */) {
    boolT othermerge = False;

    if (qh->ZEROcentrum && qh_checkzero(qh, !qh_ALL)) return;
...
void qh_postmerge(qhT *qh, const char *reason, realT maxcentrum, realT maxangle, boolT vneighbors) {
    ...
    if (qh->REPORTfreq || qh->IStracing) {
        ...
        if (qh->PRINTstatistics) qh_printallstatistics(qh, qh->ferr, "reason");
    ...
    qh->POSTmerging = True;
    if (qh->visible_list !=
        qh->facet_list) { /* first call due to qh_buildhull, multiple calls if qh.POSTmerge */
        qh->NEWfacets = True;
        qh->visible_list = qh->newfacet_list = qh->facet_list;
        FORALLnew_facets { /* all facets are new facets for qh_postmerge */
            newfacet->newfacet = True;
            if (!newfacet->simplicial)
                newfacet->newmerge = True; /* test f.vertices for 'delridge'.  'newmerge' was
                                              cleared at end of qh_all_merges */
            zinc_(Zpostfacets);
        }
        qh->newvertex_list = qh->vertex_list;
        FORALLvertices vertex->newfacet = True;
        if (qh->VERTEXneighbors) { /* a merge has occurred */
            if (qh->MERGEexact && qh->hull_dim <= qh_DIMreduceBuild)
                qh_reducevertices(
                    qh); /* qh_all_merges did not call qh_reducevertices for v.delridge */
        }
        if (!qh->PREmerge && !qh->MERGEexact) qh_flippedmerges(qh, qh->newfacet_list, &othermerges);
    }
    qh_getmergeset_initial(qh, qh->newfacet_list);
    qh_all_merges(qh, False, vneighbors); /* calls qh_reducevertices before exiting */
    FORALLnew_facets newfacet->newmerge =
        False; /* Was True if no vertex in f.vertices was 'delridge' */
} /* post_merge */
...
void qh_all_merges(qhT *qh, boolT othermerge, boolT vneighbors) {
    ...
    mergeType mergetype;
    int numcoplanar = 0, numconcave = 0, numconcavecoplanar = 0, numdegenredun = 0,
        numnewmerges = 0, numtwisted = 0;

    trace2((
        qh, qh->ferr, 2010,
        "qh_all_merges: starting to merge %d facet and %d degenerate merges for new facets f%d, othermerge? %d\n",
        qh_setsize(qh, qh->facet_mergeset), qh_setsize(qh, qh->degen_mergeset),
        getid_(qh->newfacet_list), othermerge));
    ...
    while (True) {
            ...
            while ((merge = (mergeT *)qh_setdellast(qh->facet_mergeset))) {
                ...
                if (facet1->visible || facet2->visible) {
                    trace3((
                        ...
                        facet1->id, facet1->visible, facet2->id, facet2->visible, mergetype,
                        distance, angle));
                    continue;
        ...
        if (qh->VERTEXneighbors) {                 /* at least one merge */
            ...
            if (qh->POSTmerging && qh->hull_dim >= 4) {
                ...
                if ((wasmerge || othermerge) && qh->hull_dim > 2 &&
        ...
        if (vneighbors && qh_test_vneighbors(qh /* qh.newfacet_list */)) continue;
        break;
    } /* while (True) */
    if (wasmerge || othermerge) {
        trace3((
            ...
            qh->VERTEXneighbors, qh->hull_dim, qh_DIMreduceBuild)) FORALLnew_facets {
    ...
    if (qh->IStracing >= 4 && qh->num_facets < 500) qh_printlists(qh);
} /* all_merges */
...
void qh_all_vertexmerges(qhT *qh, int apexpointid, facetT *facet, facetT **retryfacet) {
    ...
    if (retryfacet) *retryfacet = facet;
    while (qh_setsize(qh, qh->vertex_mergeset) > 0) {
        trace1((qh, qh->ferr, 1057,
        ...
        qh_merge_pinchedvertices(
            qh, apexpointid /* qh.vertex_mergeset, visible_list, newvertex_list, newfacet_list */);
        qh_update_vertexneighbors(qh); /* update neighbors of qh.newvertex_list from qh_newvertices
                                          for deleted facets on qh.visible_list */
        ...
        if (retryfacet) *retryfacet = qh_getreplacement(qh, *retryfacet);
...
void qh_appendmergeset(qhT *qh, facetT *facet, facetT *neighbor, mergeType mergetype, coordT dist,
    ...
    if ((facet->redundant && mergetype != MRGmirror) || neighbor->redundant) {
        trace3((
            ...
            facet->id, facet->redundant, neighbor->id, neighbor->redundant, facet->id, neighbor->id,
            mergetype));
        return;
    }
    ...
    if (merge->mergetype >= MRGdegen) {
        trace3((
            ...
            merge->facet1->id, merge->facet2->id, merge->mergetype, mergename,
        ...
        trace3((
            ...
            merge->facet1->id, merge->facet2->id, merge->mergetype, mergename, merge->distance,
            merge->angle, qh_setsize(qh, qh->facet_mergeset)));
    }
} /* appendmergeset */
...
void qh_appendvertexmerge(qhT *qh, vertexT *vertex, vertexT *destination, mergeType mergetype,
    ...
    trace3((qh, qh->ferr, 3034,
            "qh_appendvertexmerge: append merge v%d into v%d r%d r%d dist %2.2g type %d (%s)\n",
            vertex->id, destination->id, getid_(ridge1), getid_(ridge2), distance, merge->mergetype,
            mergename));
} /* appendvertexmerge */
...
void qh_check_dupridge(qhT *qh, facetT *facet1, realT dist1, facetT *facet2, realT dist2) {
    ...
    FOREACHvertex_(facet1->vertices) { /* The dupridge is between facet1 and facet2, so either facet
                                          can be tested */
        FOREACHvertexA_(facet1->vertices) {
            if (vertex > vertexA) { /* Test each pair once */
                ...
                /* Not quite correct.  A facet may have a dupridge and another pair of nearly
                 * adjacent vertices. */
            }
        }
    }
    ...
    if (ratio > qh_WIDEduplicate) {
        ...
        if (qh->DELAUNAY)
            qh_fprintf(qh, qh->ferr, 8145,
                       "- A bounding box for the input sites may alleviate this error.\n");
        if (!qh->ALLOWwide) qh_errexit2(qh, qh_ERRwide, facet1, facet2);
    }
} /* check_dupridge */
...
boolT qh_checkzero(qhT *qh, boolT testall) {
    ...
    if (testall)
        ...
        FORALLfacet_(facetlist) {
            ...
            if (facet->flipped || facet->dupridge || !facet->normal) goto LABELproblem;
        }
        if (qh->MERGEexact && qh->ZEROall_ok) {
            trace2(
                (qh, qh->ferr, 2011, "qh_checkzero: skip convexity check until first pre-merge\n"));
            return True;
        }
    }
    FORALLfacet_(facetlist) {
        ...
        FOREACHneighbor_i_(qh, facet) {
            ...
            if (dist >= -2 * qh->DISTround) { /* need 2x for qh_distround and 'Rn' for
                                                 qh_checkconvex, same as qh.premerge_centrum */
                qh->ZEROall_ok = False;
                if (!qh->MERGEexact || testall || dist > qh->DISTround) goto LABELnonconvex;
            }
        }
        if (!testall && horizon) {
            FOREACHvertex_(horizon->vertices) {
                if (vertex->visitid != qh->vertex_visit) {
                    ...
                    if (dist >= -2 * qh->DISTround) {
                        qh->ZEROall_ok = False;
                        if (!qh->MERGEexact || dist > qh->DISTround) goto LABELnonconvexhorizon;
                    }
                    break;
                }
            }
    ...
    trace2((qh, qh->ferr, 2012, "qh_checkzero: testall %d, facets are %s\n", testall,
            (qh->MERGEexact && !testall) ? "not concave, flipped, or dupridge" : "clearly convex"));
    return True;
...
LABELnonconvex:
    trace2((qh, qh->ferr, 2014,
            ...
            neighbor->id, vertex->id, dist));
    return False;

LABELnonconvexhorizon:
    trace2((qh, qh->ferr, 2060,
...
int qh_compare_anglemerge(const void *p1, const void *p2) {
    ...
    if (a->mergetype != b->mergetype)
        return (a->mergetype < b->mergetype
                    ? 1
                    : -1); /* select MRGcoplanar (1) before MRGconcave (3) */
    else
        return (a->angle > b->angle
                    ? 1
                    : -1); /* select coplanar merge (1.0) before sharp merge (-0.5) */
} /* compare_anglemerge */
...
int qh_compare_facetmerge(const void *p1, const void *p2) {
    ...
    if (a->mergetype != b->mergetype)
        ...
        return (a->angle > b->angle ? 1 : -1); /* if MRGanglecoplanar, select coplanar merge (1.0)
                                                  before sharp merge (-0.5) */
    else
        return (a->distance < b->distance
                    ? 1
                    : -1); /* select flat (0.0) merge before wide (1e-10) merge */
} /* compare_facetmerge */
...
void qh_degen_redundant_facet(qhT *qh, facetT *facet) {
    ...
    FOREACHneighbor_(facet) {
        if (neighbor->flipped) /* disallow merge of non-flipped into flipped, neighbor will be
                                  merged later */
            continue;
        if (neighbor->visible) {
            qh_fprintf(
...
void qh_delridge_merge(qhT *qh, ridgeT *ridge) {
    ...
    if (ridge->nonconvex) qh_copynonconvex(qh, ridge);
    ...
    FOREACHmerge_i_(qh, qh->vertex_mergeset) {
        if (merge->ridge1 == ridge || merge->ridge2 == ridge) {
            trace3((
                ...
                merge->vertex1->id, merge->vertex2->id, merge->distance, merge->ridge1->id,
                merge->ridge2->id, ridge->id));
...
vertexT *qh_find_newvertex(qhT *qh, vertexT *oldvertex, setT *vertices, setT *ridges) {
...
#endif
    FOREACHridge_(ridges) { FOREACHvertex_(ridge->vertices) vertex->seen = False; }
    ...
    qsort(SETaddr_(vertices, vertexT), (size_t)qh_setsize(qh, vertices), sizeof(vertexT *),
          qh_comparevisit);
    ...
    FOREACHridge_(ridges) qh_hashridge(qh, qh->hash_table, hashsize, ridge, oldvertex);
    FOREACHvertex_(vertices) {
...
/*-<a                             href="qh-geom2_r.htm#TOC"
  ...
  returns:
    pinched vertex (either apex or subridge), nearest vertex (subridge or neighbor vertex), and the
...
*/
vertexT *qh_findbest_pinchedvertex(qhT *qh, mergeT *merge, vertexT *apex, vertexT **nearestp,
                                   coordT *distp /* qh.newfacet_list */) {
    vertexT *vertex, **vertexp, *vertexA, **vertexAp;
    ...
    subridge = qh_vertexintersect_new(qh, merge->facet1->vertices,
                                      merge->facet2->vertices); /* new setT.  No error_exit() */
    if (qh_setsize(qh, subridge) == qh->hull_dim) {             /* duplicate vertices */
        ...
        if (bestdist > pincheddist) {
            FOREACHvertex_(subridge) {
                FOREACHvertexA_(subridge) {
                    if (vertexA->id >
                        vertex->id) { /* once per vertex pair, do not compare addresses */
        ...
        if (bestdist > pincheddist) {
            FOREACHvertexA_(subridge) {
                maybepinched = qh_neighbor_vertices(qh, vertexA,
                                                    subridge); /* subridge and apex tested above */
    ...
    *nearestp = bestvertex;
    trace2((
        ...
        qh_pointid(qh, bestpinched->point), bestpinched->id, qh_pointid(qh, bestvertex->point),
        bestvertex->id, bestdist, merge->facet1->id, merge->facet2->id));
    return bestpinched;
...
vertexT *qh_findbest_ridgevertex(qhT *qh, ridgeT *ridge, vertexT **pinchedp, coordT *distp) {
    ...
    *distp = qh_vertex_bestdist2(qh, ridge->vertices, &bestvertex, pinchedp);
    trace4((
        ...
        qh_pointid(qh, (*pinchedp)->point), (*pinchedp)->id, qh_pointid(qh, bestvertex->point),
        bestvertex->id, *distp, ridge->id, ridge->top->id, ridge->bottom->id));
    return bestvertex;
...
*/
facetT *qh_findbestneighbor(qhT *qh, facetT *facet, realT *distp, realT *mindistp,
                            realT *maxdistp) {
    ...
    *distp = REALmax;
    ...
    if (size > qh->hull_dim + qh_BESTnonconvex) {
        FOREACHridge_(facet->ridges) {
            if (ridge->nonconvex) {
                ...
                qh_findbest_test(qh, testcentrum, facet, neighbor, &bestfacet, distp, mindistp,
                                 maxdistp);
            }
        }
    }
    if (!bestfacet) {
        nonconvex = False;
        FOREACHneighbor_(facet) qh_findbest_test(qh, testcentrum, facet, neighbor, &bestfacet,
                                                 distp, mindistp, maxdistp);
    }
    if (!bestfacet) {
        qh_fprintf(qh, qh->ferr, 6095,
                   "qhull internal error (qh_findbestneighbor): no neighbors for f%d\n", facet->id);
        qh_errexit(qh, qh_ERRqhull, facet, NULL);
    }
    if (testcentrum) qh_getdistance(qh, facet, bestfacet, mindistp, maxdistp);
    trace3((
        qh, qh->ferr, 3002,
...
void qh_flippedmerges(qhT *qh, facetT *facetlist, boolT *wasmerge) {
    ...
    numdegen += qh_merge_degenredundant(qh); /* somewhat better here than after each flipped merge
                                                -- qtest.sh 10 '500 C1,2e-13 D4' 'd Qbb' */
...
void qh_forcedmerges(qhT *qh, boolT *wasmerge) {
    ...
    qh_settemppush(qh, othermerges);
    FOREACHmerge_(othermerges) {
        if (merge->mergetype != MRGdupridge) continue;
        wasdupridge = True;
        if (qh->TRACEmerge - 1 == zzval_(Ztotmerge))
            qh->qhmem.IStracing = qh->IStracing = qh->TRACElevel;
        ...
        if (dist < dist2) {
            if (facet2->flipped && !facet1->flipped &&
                dist2 < qh_WIDEdupridge *
                            (qh->ONEmerge + qh->DISTround)) { /* prefer merge of flipped facet */
                merging = facet2;
                merged = facet1;
                dist = dist2;
                mindist = mindist2;
        ...
        numdegen += qh_merge_degenredundant(
            qh); /* better here than at end -- qtest.sh 10 '500 C1,2e-13 D4' 'd Qbb' */
        if (facet1->flipped) {
            zinc_(Zmergeflipdup);
            numflip++;
        } else
    ...
    if (wasdupridge) {
        FORALLnew_facets {
            if (newfacet->dupridge) {
                ...
                if (qh_setsize(qh, newfacet->neighbors) <
                    qh->hull_dim) { /* not tested for MRGdupridge */
...
void qh_freemergesets(qhT *qh) {
    ...
    if (!SETempty_(qh->facet_mergeset) || !SETempty_(qh->degen_mergeset) ||
        !SETempty_(qh->vertex_mergeset)) {
        qh_fprintf(
            qh, qh->ferr, 6389,
...
void qh_getmergeset(qhT *qh, facetT *facetlist) {
    ...
    FORALLfacet_(facetlist) {
        ...
        FOREACHridge_(facet->ridges) {
            ...
            neighbor = otherfacet_(ridge, facet);
            if (neighbor->seen) { /* another ridge for this facet-neighbor pair was already tested
                                     in this loop */
                ridge->tested = True;
                ridge->nonconvex =
                    False; /* only one ridge is marked nonconvex per facet-neighbor pair */
            } else if (neighbor->visitid != qh->visit_id) {
                neighbor->seen = True;
                ridge->nonconvex = False;
                simplicial = False;
                if (ridge->simplicialbot && ridge->simplicialtop) simplicial = True;
                if (qh_test_appendmerge(qh, facet, neighbor, simplicial)) ridge->nonconvex = True;
                ridge->tested = True;
            }
        }
        facet->tested = True;
    ...
    nummerges = qh_setsize(qh, qh->facet_mergeset);
    if (qh->ANGLEmerge)
        qsort(SETaddr_(qh->facet_mergeset, mergeT), (size_t)nummerges, sizeof(mergeT *),
              qh_compare_anglemerge);
    else
        qsort(SETaddr_(qh->facet_mergeset, mergeT), (size_t)nummerges, sizeof(mergeT *),
...
void qh_getmergeset_initial(qhT *qh, facetT *facetlist) {
    ...
    FORALLfacet_(facetlist) {
        ...
        FOREACHneighbor_(facet) {
            if (neighbor->visitid != qh->visit_id) {
                simplicial =
                    False; /* ignores r.simplicialtop/simplicialbot.  Need to test horizon facets */
                if (facet->simplicial && neighbor->simplicial) simplicial = True;
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  returns:
    ...
    True and !iscoplanar, if should merge a pinched vertex of a dupridge
      qh.vertex_mergeset contains one or more MRGsubridge with a pinched vertex and a nearby,
  neighboring vertex qh.facet_mergeset is empty
...
boolT qh_getpinchedmerges(qhT *qh, vertexT *apex, coordT maxdupdist,
                          boolT *iscoplanar /* qh.newfacet_list, qh.vertex_mergeset */) {
    ...
    *iscoplanar = False;
    ...
    FOREACHmerge_(qh->facet_mergeset) { /* read-only */
        ...
        pinched =
            qh_findbest_pinchedvertex(qh, merge, apex, &nearest, &dist /* qh.newfacet_list */);
        if (pinched == apex &&
            dist <
                qh_RATIOcoplanarapex * bestdist) { /* prefer coplanar apex since it always works */
    ...
    if (bestmerge && bestdist < maxdupdist) {
        ...
        if (ratio > qh_WIDEpinched) {
            if (bestmerge->facet1->mergehorizon ||
                bestmerge->facet2
                    ->mergehorizon) { /* e.g., rbox 175 C3,2e-13 t1539182828 | qhull d */
                trace1((
                    qh, qh->ferr, 1051,
                    ...
                    ratio, bestpinched->id, bestvertex->id, bestdist, bestmerge->facet1->id,
                    bestmerge->facet2->id));
            } else {
                qh_fprintf(
                    qh, qh->ferr, 7081,
                    ...
                    bestpinched->id, bestvertex->id, bestdist, ratio, bestmerge->facet1->id,
                    bestmerge->facet2->id);
            }
        } else {
            if (bestpinched == apex) {
                trace2((
                    ...
                    qh_pointid(qh, apex->point), apex->id, bestvertex->id,
                    bestdist * qh_RATIOcoplanarapex));
                qh->coplanar_apex = apex->point;
                *iscoplanar = True;
                result = True;
            } else if (qh_setin(bestmerge->facet1->vertices, bestpinched) !=
                       qh_setin(bestmerge->facet2->vertices,
                                bestpinched)) { /* pinched in one facet but not the other facet */
                trace2((
                    qh, qh->ferr, 2064,
                ...
                qh_appendvertexmerge(qh, bestpinched, bestvertex, MRGsubridge, bestdist, NULL,
                                     NULL);
                result = True;
            } else {
                trace2((
...
boolT qh_hasmerge(setT *mergeset, mergeType type, facetT *facetA, facetT *facetB) {
    ...
    FOREACHmerge_(mergeset) {
        if (merge->mergetype == type) {
            if (merge->facet1 == facetA && merge->facet2 == facetB) return True;
            if (merge->facet1 == facetB && merge->facet2 == facetA) return True;
        }
    }
    return False;
} /* hasmerge */
...
void qh_makeridges(qhT *qh, facetT *facet) {
    ...
    FOREACHneighbor_i_(qh, facet) {
        if (neighbor == qh_MERGEridge)
            ...
            ridge->vertices =
                qh_setnew_delnthsorted(qh, facet->vertices, qh->hull_dim, neighbor_i, 0);
            ...
            if (facet->tested && !mergeridge) ridge->tested = True;
      ...
      if (facet->toporient ^ (skip1 & 0x1) ^ flip) {
        ...
        ridge->simplicialbot= True;
            ...
            if (qh->ridge_id == qh->traceridge_id) qh->traceridge = ridge;
        }
    }
    if (mergeridge) {
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  returns:
    ...
    if allmerges is true
      make ridges for facets with dupridges as marked by qh_MERGEridge and both sides
  facet->dupridge removes qh_MERGEridge from neighbor sets
...
void qh_mark_dupridges(qhT *qh, facetT *facetlist, boolT allmerges) {
        ...
        if (facet->dupridge) {
            FOREACHneighbor_(facet) {
                ...
                if (neighbor->dupridge) {
                    if (!qh_setin(neighbor->neighbors,
                                  facet)) { /* i.e., it is qh_MERGEridge, neighbors are distinct */
                        qh_appendmergeset(qh, facet, neighbor, MRGdupridge, 0.0, 1.0);
                        facet->mergeridge2 = True;
                        facet->mergeridge = True;
                        nummerge++;
                    } else if (qh_setequal(facet->vertices,
                                           neighbor->vertices)) { /* neighbors are the same except
                                                                     for horizon and qh_MERGEridge,
                                                                     see QH7085 */
                        trace3((
                            qh, qh->ferr, 3043,
    ...
    FORALLfacet_(facetlist) {
        if (facet->mergeridge && !facet->mergeridge2) qh_makeridges(qh, facet);
    }
    trace3((qh, qh->ferr, 3075,
    ...
    FOREACHmerge_(qh->facet_mergeset) {        /* restore the missing neighbors */
        if (merge->mergetype == MRGdupridge) { /* only between simplicial facets */
            if (merge->facet2->mergeridge2 && qh_setin(merge->facet2->neighbors, merge->facet1)) {
                /* Due to duplicate or multiple subridges, e.g., ../eg/qtest.sh t712682 '200 s
                   W1e-13  C1,1e-13 D5' 'd' merge->facet1:    - neighboring facets: f27779 f59186
                   f59186 f59186 MERGEridge f59186 merge->facet2:    - neighboring facets: f27779
                   f59100 f59100 f59100 f59100 f59100 or, ../eg/qtest.sh 100 '500 s W1e-13 C1,1e-13
                   D4' 'd' both facets will be degenerate after merge, consider for special case
                   handling
                */
                qh_fprintf(
                    qh, qh->ferr, 6361,
...
void qh_maybe_duplicateridge(qhT *qh, ridgeT *ridgeA) {
    ...
    if (qh->hull_dim < 3) return;

    for (neighbor = ridgeA->top, i = 0; i < 2; neighbor = ridgeA->bottom, i++) {
        if (!neighbor->simplicial &&
            neighbor->nummerge > 0) { /* skip degenerate neighbors with both new and old vertices
                                         that will be merged */
            FOREACHridge_(neighbor->ridges) {
                    ...
                    if (SETelem_(ridge->vertices, last) == SETelem_(ridgeA->vertices, last)) {
                        ...
                        if (k == last) {
                            ...
                            trace2((
                                ...
                                ridgeA->top->id, ridgeA->bottom->id, ridge->top->id,
                                ridge->bottom->id));
                            qh_appendvertexmerge(qh, pinched, vertex, MRGvertices, dist, ridgeA,
                                                 ridge);
                            ridge->mergevertex =
                                True; /* disables check for duplicate vertices in qh_checkfacet */
...
void qh_maybe_duplicateridges(qhT *qh, facetT *facet) {
    ...
    if (qh->hull_dim < 3 || !qh->CHECKduplicates) return;

    FOREACHridge_i_(qh, facet->ridges) {
        otherfacet = otherfacet_(ridge, facet);
        if (otherfacet->degenerate || otherfacet->redundant || otherfacet->dupridge ||
            otherfacet->flipped) /* will merge */
            continue;
        for (i = ridge_i + 1; i < ridge_n; i++) {
            ...
            if (SETelem_(ridge->vertices, last_v) ==
                SETelem_(ridge2->vertices, last_v)) { /* SETfirst is likely to be the same */
                if (SETfirst_(ridge->vertices) == SETfirst_(ridge2->vertices)) {
                    for (k = 1; k < last_v; k++) {
                        if (SETelem_(ridge->vertices, k) != SETelem_(ridge2->vertices, k)) break;
                    }
                    if (k == last_v) {
                        vertex = qh_findbest_ridgevertex(qh, ridge, &pinched, &dist);
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  qh_merge_degenredundant(qh)
    ...
    qh.degen_mergeset contains merges from  qh_test_degen_neighbors, qh_test_redundant_neighbors,
  and qh_degen_redundant_facet
    ...
    while ((merge = (mergeT *)qh_setdellast(qh->degen_mergeset))) {
        ...
        if (mergetype == MRGredundant) {
            ...
            trace2((qh, qh->ferr, 2025,
                    "qh_merge_degenredundant: merge redundant f%d into f%d (arg f%d)\n", facet1->id,
                    facet3->id, facet2->id));
            ...
            if (!(size = qh_setsize(qh, facet1->neighbors))) {
                ...
                qh_mergefacet(qh, facet1, bestneighbor, mergetype, &mindist, &maxdist,
                              !qh_MERGEapex);
                nummerges++;
                if (qh->PRINTstatistics) {
                    zinc_(Zdegen);
...
void qh_merge_pinchedvertices(qhT *qh, int apexpointid /* qh.newfacet_list */) {
    ...
    while ((merge = qh_next_vertexmerge(
                qh /* qh.vertex_mergeset */))) { /* only one at a time from qh_getpinchedmerges */
        if (qh->TRACEmerge - 1 == zzval_(Ztotmerge))
            qh->qhmem.IStracing = qh->IStracing = qh->TRACElevel;
...
void qh_mergecycle(qhT *qh, facetT *samecycle, facetT *newfacet) {
    ...
    if (qh->REPORTfreq2 && qh->POSTmerging) {
        if (zzval_(Ztotmerge) > qh->mergereport + qh->REPORTfreq2) qh_tracemerging(qh);
    }
#ifndef qh_NOtrace
    if (qh->TRACEmerge == zzval_(Ztotmerge)) qh->qhmem.IStracing = qh->IStracing = qh->TRACElevel;
    ...
    if (qh->IStracing >= 4) {
        ...
        FORALLsame_cycle_(samecycle) qh_fprintf(qh, qh->ferr, 8070, " f%d", same->id);
        qh_fprintf(qh, qh->ferr, 8071, "\n");
    }
    if (qh->IStracing >= 4) qh_errprint(qh, "MERGING CYCLE", samecycle, newfacet, NULL, NULL);
#endif /* !qh_NOtrace */
    ...
    if (qh->CHECKfrequently) qh_checkdelridge(qh);
    if (!qh->VERTEXneighbors) qh_vertexneighbors(qh);
    apex = SETfirstt_(samecycle->vertices, vertexT);
    ...
    if (!newfacet->newfacet) qh_newvertices(qh, newfacet->vertices);
...
void qh_mergecycle_all(qhT *qh, facetT *facetlist, boolT *wasmerge) {
    ...
    for (facet = facetlist; facet && (nextfacet = facet->next); facet = nextfacet) {
        ...
        if (facet->f.samecycle == facet) {
            ...
            for (same = facet->f.samecycle; same;            /* FORALLsame_cycle_(facet) */
                ...
                if (same->cycledone || same->visible) qh_infiniteloop(qh, same);
                same->cycledone = True;
                if (same->normal) {
            ...
            if (nummerge > qh_MAXnummerge)
                ...
                horizon->nummerge = (short unsigned int)
                    nummerge; /* limited to 9 bits by qh_MAXnummerge, -Wconversion */
    ...
    if (cycles) {
        FORALLnew_facets {
            /* qh_maybe_duplicateridges postponed since qh_mergecycle_ridges deletes ridges without
             * calling qh_delridge_merge */
...
void qh_mergecycle_facets(qhT *qh, facetT *samecycle, facetT *newfacet) {
    ...
    if (newfacet->center && qh_setsize(qh, newfacet->vertices) <= qh->hull_dim + qh_MAXnewcentrum) {
...
void qh_mergecycle_neighbors(qhT *qh, facetT *samecycle, facetT *newfacet) {
    ...
    FORALLsame_cycle_(samecycle) {
        if (same->visitid == samevisitid || same->visible) qh_infiniteloop(qh, samecycle);
        same->visitid = samevisitid;
    }
    newfacet->visitid = ++qh->visit_id;
    trace4(
        (qh, qh->ferr, 4031, "qh_mergecycle_neighbors: delete shared neighbors from newfacet\n"));
...
void qh_mergecycle_ridges(qhT *qh, facetT *samecycle, facetT *newfacet) {
    ...
    FOREACHridge_(newfacet->ridges) {
        ...
        if (neighbor->visitid == samevisitid) SETref_(ridge) = NULL; /* ridge free'd below */
    }
    ...
    FORALLsame_cycle_(samecycle) {
        ...
        if (same->ridges) qh_settruncate(qh, same->ridges, 0);
        ...
        FOREACHneighbor_i_(qh, same) { /* note: !newfact->simplicial */
            if (neighbor->visitid != samevisitid && neighbor->simplicial) {
                ridge = qh_newridge(qh);
                ridge->vertices =
                    qh_setnew_delnthsorted(qh, same->vertices, qh->hull_dim, neighbor_i, 0);
...
void qh_mergecycle_vneighbors(qhT *qh, facetT *samecycle, facetT *newfacet) {
    ...
    trace4(
        (qh, qh->ferr, 4035, "qh_mergecycle_vneighbors: update vertex neighbors for newfacet\n"));
...
void qh_mergefacet(qhT *qh, facetT *facet1, facetT *facet2, mergeType mergetype, realT *mindist,
    ...
    if (qh->build_cnt >= qh->RERUN) {
        if (mindist && (-*mindist > qh->TRACEdist || *maxdist > qh->TRACEdist)) {
            ...
            qh_fprintf(
                ...
                zzval_(Ztotmerge), fmax_(-*mindist, *maxdist), facet1->id, facet2->id, mergetype,
                mergename, qh->furthest_id);
    ...
    if (qh->IStracing >= 2) {
        ...
        qh_fprintf(
            ...
            zzval_(Ztotmerge), facet1->id, facet2->id, mergetype, mergename, mergemin, mergemax,
            qh->max_outside);
    }
#endif /* !qh_NOtrace */
    if (!qh->ALLOWwide && mindist) {
        mintwisted = qh_WIDEmaxoutside *
                     qh->ONEmerge; /* same as qh_merge_twisted and qh_check_maxout (poly2) */
        maximize_(mintwisted, facet1->maxoutside);
        maximize_(mintwisted, facet2->maxoutside);
        if (*maxdist > mintwisted || -*mindist > mintwisted) {
            ...
            if (vertexdist > mintwisted) {
                qh_fprintf(
                    ...
                    facet1->id, facet2->id, mergetype, mergename, *maxdist, *maxdist / onemerge,
                    *mindist, -*mindist / onemerge, vertexdist);
            } else {
                qh_fprintf(
                    qh, qh->ferr, 6348,
    ...
    if (qh->IStracing >= 4) qh_errprint(qh, "MERGING", facet1, facet2, NULL, NULL);
    if (mindist) {
        ...
        if (!facet2->keepcentrum && (*maxdist > qh->WIDEfacet || *mindist < -qh->WIDEfacet)) {
    ...
    if (nummerge >= qh_MAXnummerge)
        ...
        facet2->nummerge =
            (short unsigned int)nummerge; /* limited to 9 bits by qh_MAXnummerge, -Wconversion */
    ...
    if (qh->hull_dim > 2 && qh_setsize(qh, facet1->vertices) == qh->hull_dim)
        ...
        if (!facet2->newfacet) qh_newvertices(qh, facet2->vertices);
    }
    ...
    if (mergetype != MRGcoplanarhorizon) {
        ...
        qh_test_degen_neighbors(
            qh, facet1); /* after qh_test_redundant_neighbors since MRGdegen more difficult than
                            MRGredundant and before qh_willdelete which clears facet1.neighbors */
        qh_degen_redundant_facet(qh, facet2); /* may occur in qh_merge_pinchedvertices, e.g., rbox
                                                 175 C3,2e-13 D4 t1545228104 | qhull d */
...
void qh_mergesimplex(qhT *qh, facetT *facet1, facetT *facet2, boolT mergeapex) {
    ...
    if (mergeapex) {
        opposite = SETfirstt_(facet1->vertices,
                              vertexT); /* apex is opposite facet2.  It has the last vertex id */
    ...
    FOREACHvertex_(facet1->vertices) {
        if (vertex == opposite && isnew)
            ...
            qh_setdel(vertex->neighbors, facet1);
            if (!SETsecond_(vertex->neighbors)) qh_mergevertex_del(qh, vertex, facet1, facet2);
        }
    }
    ...
    FOREACHridge_(facet1->ridges) {
        ...
        if (otherfacet == facet2) {
            ...
            qh_delridge_merge(qh, ridge);         /* expensive in high-d, could rebuild */
            ridgep--;                             /* deleted this ridge, repeat with next ridge*/
            qh_setdel(facet2->neighbors, facet1); /* a simplicial facet may have duplicate
                                                     neighbors, need to delete each one */
            ...
            trace4(
                (qh, qh->ferr, 4059,
                 ...
                 ridge->id, otherfacet->id, facet2->id, (otherfacet->visitid != qh->visit_id),
                 (SETfirstt_(otherfacet->neighbors, facetT) == facet1)));
            qh_setappend(qh, &facet2->ridges, ridge);
...
void qh_mergevertex_neighbors(qhT *qh, facetT *facet1, facetT *facet2) {
    ...
    trace4((qh, qh->ferr, 4042,
            "qh_mergevertex_neighbors: merge vertex neighborset for f%d into f%d\n", facet1->id,
            facet2->id));
    if (qh->tracevertex) {
        qh_fprintf(qh, qh->ferr, 8081,
                   "qh_mergevertex_neighbors: of f%d into f%d at furthest p%d f0= %p\n", facet1->id,
                   facet2->id, qh->furthest_id, qh->tracevertex->neighbors->e[0].p);
    ...
    if (qh->tracevertex) qh_errprint(qh, "TRACE", NULL, NULL, NULL, qh->tracevertex);
...
void qh_mergevertices(qhT *qh, setT *vertices1, setT **vertices2) {
    ...
    while (*vertex2) qh_setappend(qh, &mergedvertices, *vertex2++);
...
setT *qh_neighbor_vertices(qhT *qh, vertexT *vertexA, setT *subridge) {
    ...
    qh->visit_id++;
    FOREACHneighbor_(vertexA) neighbor->visitid = qh->visit_id;
    qh->vertex_visit++;
    vertexA->visitid = qh->vertex_visit;
    FOREACHvertex_(subridge) { vertex->visitid = qh->vertex_visit; }
        ...
        FOREACHridge_(facet->ridges) {
            ...
            if (neighbor->visitid == qh->visit_id) {
                ...
                if (SETfirst_(ridge->vertices) == vertexA) {
                    ...
                    if (second->id >= vertexA->id &&
                        last->id <= vertexA->id) { /* vertices inverse sorted by id */
...
mergeT *qh_next_vertexmerge(qhT *qh /* qh.vertex_mergeset */) {
    ...
    FOREACHmerge_i_(qh, qh->vertex_mergeset) {
        ...
        if (merge->vertex1->deleted || merge->vertex2->deleted) {
            trace3((
                ...
                merge->vertex1->id, merge->vertex1->deleted, merge->vertex2->id,
                merge->vertex2->deleted, getid_(merge->ridge1), getid_(merge->ridge2)));
            qh_drop_mergevertex(qh, merge);
            qh_setdelnth(qh, qh->vertex_mergeset, merge_i);
            merge_i--;
            merge_n--;
    ...
    if (best_i >= 0) {
        ...
        if (bestdist / qh->ONEmerge > qh_WIDEpinched) {
            if (merge->mergetype == MRGvertices) {
                if (merge->ridge1->top == merge->ridge2->bottom &&
                    merge->ridge1->bottom == merge->ridge2->top)
                    qh_fprintf(
                        qh, qh->ferr, 6391,
                        ...
                        merge->ridge1->id, merge->ridge2->id, merge->ridge1->top->id,
                        merge->ridge1->bottom->id, merge->vertex1->id, merge->vertex2->id, bestdist,
                        bestdist / qh->ONEmerge);
                else
                    qh_fprintf(
                        qh, qh->ferr, 6381,
                        ...
                        merge->ridge1->id, merge->ridge2->id, merge->vertex1->id,
                        merge->vertex2->id, bestdist, bestdist / qh->ONEmerge);
            } else {
                qh_fprintf(
                    qh, qh->ferr, 6208,
            ...
            /* it may be possible to find a different vertex, after other vertex merges have
             * occurred */
...
facetT *qh_opposite_horizonfacet(qhT *qh, mergeT *merge, vertexT **opposite) {
    ...
    if (!merge->facet1->simplicial || !merge->facet2->simplicial ||
        (!merge->facet1->mergehorizon && !merge->facet2->mergehorizon)) {
        qh_fprintf(
            qh, qh->ferr, 6273,
    ...
    if (neighbor_i == -1) neighbor_i = qh_setindex(otherfacet->neighbors, qh_MERGEridge);
    if (neighbor_i == -1) {
        qh_fprintf(
            qh, qh->ferr, 6238,
...
boolT qh_remove_extravertices(qhT *qh, facetT *facet) {
    ...
    FOREACHridge_(facet->ridges) { FOREACHvertex_(ridge->vertices) vertex->seen = True; }
    FOREACHvertex_(facet->vertices) {
        if (!vertex->seen) {
            foundrem = True;
            zinc_(Zremvertex);
...
void qh_remove_mergetype(qhT *qh, setT *mergeset, mergeType type) {
    ...
    FOREACHmerge_i_(qh, mergeset) {
        if (merge->mergetype == type) {
            trace3((qh, qh->ferr, 3037,
                    "qh_remove_mergetype: remove merge f%d f%d v%d v%d r%d r%d dist %2.2g type %d",
                    getid_(merge->facet1), getid_(merge->facet2), getid_(merge->vertex1),
                    getid_(merge->vertex2), getid_(merge->ridge1), getid_(merge->ridge2),
                    merge->distance, type));
...
void qh_rename_adjacentvertex(qhT *qh, vertexT *oldvertex, vertexT *newvertex, realT dist) {
    ...
    if (qh->IStracing >= 4 || oldvertex->id == qh->tracevertex_id ||
    ...
    FOREACHneighbor_(oldvertex) {
        if (neighbor->simplicial) {
            if (qh_setin(neighbor->vertices, newvertex)) {
                ...
                qh_makeridges(qh, neighbor); /* no longer simplicial, nummerge==0, skipped by
                                                qh_maybe_duplicateridge */
            } else {
                qh_replacefacetvertex(qh, neighbor, oldvertex, newvertex);
                qh_setunique(qh, &newvertex->neighbors, neighbor);
                qh_newvertices(
                    qh, neighbor->vertices); /* for qh_update_vertexneighbors of vertex neighbors */
            }
        }
    }
    ridges = qh_vertexridges(qh, oldvertex, qh_ALL);
    if (istrace) {
        FOREACHridge_(ridges) { qh_printridge(qh, qh->ferr, ridge); }
    }
    FOREACHneighbor_(oldvertex) {
        if (!neighbor->simplicial) {
            ...
            if (qh->newfacet_list == qh->facet_tail) {
                qh_removefacet(qh, neighbor); /* add a neighbor to newfacet_list so that
                                                 qh_partitionvisible has a newfacet */
                qh_appendfacet(qh, neighbor);
    ...
    if (oldvertex->deleted && !oldvertex->partitioned) {
        ...
        trace2((
            ...
            qh_partitioncoplanar(qh, oldvertex->point, maxfacet, NULL,
                                 !qh_ALL); /* faster with maxdist2, otherwise duplicates distance
                                              tests from maxdist2/dist2 */
...
vertexT *qh_rename_sharedvertex(qhT *qh, vertexT *vertex, facetT *facet) {
    ...
    if (neighborA) { /* avoid warning */
        ...
        trace2((qh, qh->ferr, 2037,
                "qh_rename_sharedvertex: p%d(v%d) is shared by f%d(%d ridges) and f%d\n",
                qh_pointid(qh, vertex->point), vertex->id, facet->id, qh_setsize(qh, ridges),
                neighborA->id));
        zinc_(Zintersectnum);
        ...
        if ((newvertex = qh_find_newvertex(qh, vertex, vertices, ridges)))
            qh_renamevertex(qh, vertex, newvertex, ridges, facet,
                            neighborA); /* ridges invalidated */
        qh_settempfree(qh, &vertices);
        qh_settempfree(qh, &ridges);
    }
    return newvertex;
...
void qh_renamevertex(qhT *qh, vertexT *oldvertex, vertexT *newvertex, setT *ridges,
    ...
    if (qh->IStracing >= 2 || oldvertex->id == qh->tracevertex_id ||
        ...
        qh_fprintf(
            ...
            oldvertex->id, newvertex->id, qh_setsize(qh, ridges), getid_(oldfacet),
            getid_(neighborA));
    }
#endif
    FOREACHridge_(ridges) {
        if (qh_renameridgevertex(qh, ridge, oldvertex,
                                 newvertex)) { /* ridge is deleted if False, invalidating ridges */
            topsize = qh_setsize(qh, ridge->top->vertices);
            bottomsize = qh_setsize(qh, ridge->bottom->vertices);
            if (topsize < qh->hull_dim || (topsize == qh->hull_dim && !ridge->top->simplicial &&
                                           qh_setin(ridge->top->vertices, newvertex))) {
                trace4((
                    qh, qh->ferr, 4070,
            ...
            } else if (bottomsize < qh->hull_dim ||
                       (bottomsize == qh->hull_dim && !ridge->bottom->simplicial &&
                        qh_setin(ridge->bottom->vertices, newvertex))) {
                trace4((
                    qh, qh->ferr, 4071,
    ...
    if (!oldfacet) {
        ...
        FOREACHneighbor_(oldvertex) {
            if (neighbor->simplicial) {
                qh_degen_redundant_facet(
                    qh, neighbor); /* e.g., rbox 175 C3,2e-13 D4 t1545235541 | qhull d */
            } else {
                if (istrace)
                    qh_fprintf(
                        qh, qh->ferr, 4080,
                ...
                qh_setdelsorted(
                    neighbor->vertices,
                    oldvertex); /* if degenerate, qh_degen_redundant_facet will add to mergeset */
                if (qh_remove_extravertices(qh, neighbor))
                    neighborp--; /* neighbor deleted from oldvertex neighborset */
                qh_degen_redundant_facet(
                    qh, neighbor); /* either direction may be redundant, faster if combine? */
        ...
        if (istrace || qh->IStracing >= 1)
            qh_fprintf(qh, qh->ferr, 3040,
                       "qh_renamevertex: renaming pinched v%d to v%d between f%d and f%d\n",
        ...
        if (qh_remove_extravertices(qh, neighborA)) qh_degen_redundant_facet(qh, neighborA);
    }
    if (oldfacet) qh_degen_redundant_facet(qh, oldfacet);
} /* renamevertex */
...
boolT qh_test_centrum_merge(qhT *qh, facetT *facet, facetT *neighbor, realT angle, boolT okangle) {
    ...
    if (dist > qh->centrum_radius)
        ...
        iscoplanar = True;
    if (!neighbor->center) neighbor->center = qh_getcentrum(qh, neighbor);
    zzinc_(Zcentrumtests);
    ...
    if (dist2 > qh->centrum_radius)
        ...
        iscoplanar = True;
    if (!isconcave && (!iscoplanar || (qh->MERGEexact && !qh->POSTmerging))) return False;
...
void qh_test_degen_neighbors(qhT *qh, facetT *facet) {
    ...
    FOREACHneighbor_(facet) {
        ...
        if (neighbor->degenerate || neighbor->redundant ||
            neighbor->dupridge) /* will merge or delete */
            continue;
...
boolT qh_test_nonsimplicial_merge(qhT *qh, facetT *facet, facetT *neighbor, realT angle,
                                  boolT okangle) {
    coordT dist, mindist, maxdist, mindist2, maxdist2, dist2, maxoutside, clearlyconcave, minvertex,
        clearlyconvex, mergedist, coplanarcentrum;
    ...
    minvertex =
        fmax_(-qh->min_vertex,
              qh->MAXcoplanar); /* non-negative, not available per facet, not used for iscoplanar */
    clearlyconvex = qh_RATIOconvexmerge * minvertex; /* must be convex for MRGtwisted */
    if (qh->MERGEexact && !qh->POSTmerging &&
        (facet->nummerge > qh_MAXcoplanarcentrum || neighbor->nummerge > qh_MAXcoplanarcentrum))
    ...
    if (dist2 > coplanarcentrum)
        ...
        if (maybeconvex) return False; /* both centrums clearly convex */
        maybeconvex = True;
    }
    if (isconcave) {
        if (!okangle && qh->ANGLEmerge) {
    ...
    if (maybeconvex) {
        if (dist < -clearlyconvex) {
            ...
            maxdist2 =
                dist2; /* neighbor centrum clearly convex, no need to test its vertex distance */
            mindist2 = dist2;
    ...
    if (maxdist > clearlyconcave || maxdist2 > clearlyconcave)
        ...
        if (maxdist > maxoutside || maxdist2 > maxoutside) isconcave = True; /* MRGtwisted */
    }
    if (!isconcave && qh->MERGEexact && !qh->POSTmerging) return False;
    if (isconcave && !iscoplanar) {
        if (maxdist < maxoutside &&
            (-qh->MAXcoplanar || (maxdist2 < maxoutside && mindist2 >= -qh->MAXcoplanar)))
            iscoplanar = True; /* MRGconcavecoplanar */
    } else if (!iscoplanar) {
        if (mindist >= -qh->MAXcoplanar || mindist2 >= -qh->MAXcoplanar)
            iscoplanar = True; /* MRGcoplanar */
    }
    if (!isconcave && !iscoplanar) return False;
    if (!okangle && qh->ANGLEmerge) {
        angle = qh_getangle(qh, facet->normal, neighbor->normal);
        zinc_(Zangletests);
    }
    if (isconcave && maybeconvex) {
        ...
        trace0((
            ...
            facet->id, getid_(maxvertex), neighbor->id, getid_(maxvertex2), maxdist, maxdist2,
            angle, qh->furthest_id));
...
void qh_test_redundant_neighbors(qhT *qh, facetT *facet) {
    ...
    trace4((qh, qh->ferr, 4022,
            "qh_test_redundant_neighbors: test neighbors of f%d vertex_visit %d\n", facet->id,
            qh->vertex_visit + 1));
    if ((size = qh_setsize(qh, facet->neighbors)) < qh->hull_dim) {
        qh_appendmergeset(qh, facet, facet, MRGdegen, 0.0, 1.0);
        trace2((qh, qh->ferr, 2017,
        ...
        FOREACHvertex_(newfacet->vertices) {
            FOREACHneighbor_(vertex) {
                ...
                if (qh_test_appendmerge(qh, newfacet, neighbor,
                                        False)) /* ignores optimization for simplicial ridges */
                    nummerges++;
            }
        }
    }
...
void qh_tracemerge(qhT *qh, facetT *facet1, facetT *facet2, mergeType mergetype) {
    ...
    if (mergetype > 0 && mergetype < sizeof(mergetypes) / sizeof(char *))
        ...
        mergename = mergetypes[MRGnone];
    if (qh->IStracing >= 4) qh_errprint(qh, "MERGED", facet2, NULL, NULL, NULL);
...
/*-<a                             href="qh-merge_r.htm#TOC"
  ...
  notes:
    ...
    skips last neighbor, unless allneighbors.  For new facets, the last neighbor shares ridges with
  adjacent neighbors if the last neighbor is not simplicial, it will have ridges for its simplicial
  neighbors Use allneighbors when a new cone is attached to an existing convex hull similar to
  qh_neighbor_vertices
...
setT *qh_vertexridges(qhT *qh, vertexT *vertex, boolT allneighbors) {
    ...
    if (qh->PRINTstatistics || qh->IStracing) {
        ...
        trace3(
            (qh, qh->ferr, 3011, "qh_vertexridges: found %d ridges for v%d\n", size, vertex->id));
    }
    return ridges;
} /* vertexridges */
    ...
    FOREACHridge_(facet->ridges) {
        ...
        if (neighbor->visitid == qh->visit_id) {
            if (SETfirst_(ridge->vertices) == vertex) {
                ...
                second = SETsecondt_(ridge->vertices, vertexT);
                last = SETelemt_(ridge->vertices, last_i, vertexT);
                if (second->id >= vertex->id &&
                    last->id <= vertex->id) { /* vertices inverse sorted by id */
...
void qh_willdelete(qhT *qh, facetT *facet, facetT *replace) {
    ...
    if (facet->ridges) SETfirst_(facet->ridges) = NULL;
    if (facet->neighbors) SETfirst_(facet->neighbors) = NULL;
} /* willdelete */
...
void qh_checkdelridge(qhT *qh /* qh.visible_facets, vertex_mergeset */){QHULL_UNUSED(qh)} boolT
    qh_checkzero(qhT *qh, boolT testall) {
    QHULL_UNUSED(qh)
    QHULL_UNUSED(testall)

    return True;
}
void qh_freemergesets(qhT *qh) { QHULL_UNUSED(qh) }
void qh_initmergesets(qhT *qh) { QHULL_UNUSED(qh) }
void qh_merge_pinchedvertices(qhT *qh, int apexpointid /* qh.newfacet_list */) {
    QHULL_UNUSED(qh)
    QHULL_UNUSED(apexpointid)
}
